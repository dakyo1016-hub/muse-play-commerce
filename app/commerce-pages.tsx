...
type Kind="items"|"ranking"|"quest"|"my"|"bag"|"try-on";
...
export function CommerceNav({active,cartCount=0}:{active:Kind;cartCount?:number}){
 const entries:[string,string][]=[["ITEMS","/items"],["RANKING","/ranking"],["★ PLAY","/play"],["AI TRY-ON","/try-on"],["QUEST","/quest"],["MY","/my"]];
...
function ItemsPage(){
 ...
 return <><CommerceNav active="items"/><main className="section-page"><Header eyebrow="ITEM ARCHIVE · 12 PRODUCTS" title="취향 태그로 찾는 아이템" copy="실제 상품 데이터를 스타일 태그로 탐색하고, 마음에 든 아이템은 PLAY에서 바로 조합해보세요."/><div className="archive-filter">{["ALL","TOP","BOTTOM","SHOES","BAG"].map(value=><button key={value} className={filter===value?"active":""} onClick={()=>setFilter(value)}>{value}</button>)}</div><section className="archive-grid">{visible.map(item=><ItemCard key={item.id} item={item}/>)}</section></main></>
}
...
function BagPage(){
 ...
 return <><CommerceNav active="bag" cartCount={items.length}/><main className="section-page"><Header eyebrow="SHOPPING BAG" title={ordered?"LOOK을 장바구니에 담았어요":"선택한 LOOK을 확인하세요"} copy={ordered?"결제는 기존 커머스의 안전한 체크아웃에서 이어집니다.":"PLAY에서 고른 상품을 개별로 제외하거나 세트로 구매할 수 있습니다."}/>{ordered?<section className="bag-success"><span>✓</span><h2>장바구니 저장 완료</h2><p>{items.length}개 아이템 · {won(sum)}</p><a href="/items">쇼핑 계속하기 →</a></section>:items.length?<section className="bag-layout"><div>{items.map(item=><article key={item.id}><img src={item.image} alt={item.name}/><div><small>{item.brand} · {item.category}</small><b>{item.name}</b><span>{item.tags.map(tag=>`#${tag}`).join("  ")}</span></div><strong>{won(item.price)}</strong><button aria-label={`${item.name} 삭제`} onClick={()=>setIds(ids.filter(id=>id!==item.id))}>×</button></article>)}</div><aside><span>ORDER SUMMARY</span><p><small>ITEMS</small><b>{items.length}</b></p><p><small>DELIVERY</small><b>FREE</b></p><footer><small>TOTAL</small><strong>{won(sum)}</strong></footer><button onClick={()=>setOrdered(true)}>선택한 LOOK 장바구니 담기 →</button></aside></section>:<section className="bag-empty"><span>BAG 00</span><h2>아직 담긴 LOOK이 없어요.</h2><p>PLAY에서 상황에 맞는 코디를 완성하거나 아이템을 먼저 둘러보세요.</p><div><a href="/play">PLAY 시작하기 →</a><a href="/items">아이템 보기</a></div></section>}</main></>
}
