...
export async function POST(request:Request){
 try{
  ...
  const prompt=`Create one highly realistic full-body fashion try-on photograph in a strict vertical 3:4 composition. The first reference is the exact adult customer. Preserve the same recognizable face, facial features, body proportions, skin tone, hairstyle and identity. The complete face, full hair and head must be clearly visible, front-facing and unobscured. Dress the person naturally in these exact selected products: ${selected.map(item=>item.label).join(", ")}. Product references follow in the same order. Preserve garment colors, silhouette, texture, logos and construction details. Make the layering physically plausible, properly fitted and photorealistic. Render smooth continuous human anatomy and natural seamless skin at the shoulders, elbows, wrists, hips, knees and ankles. Never show doll joints, ball joints, articulation seams, hinges, sockets, mannequin parts or plastic toy surfaces, even when the reference has a stylized appearance. Pull the camera back and show the entire person from the top of the hair to the soles of both shoes, with 10 percent breathing room above the head and below the feet. Do not crop, zoom in, cut off the face, head, legs, shoes or any body part. Neutral light-gray studio background, soft editorial lighting, natural centered standing pose, no extra people, no text, no collage, no duplicate body parts.`;
