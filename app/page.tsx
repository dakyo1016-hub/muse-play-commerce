...
function LookBoard({items,label="SELECTED LOOK",emptyCopy="카테고리별 상품을 선택하면 이곳에 LOOK이 쌓입니다."}:{items:Product[];label?:string;emptyCopy?:string}){
 ...
 return <section className="look-board"><header><b>{label}</b><span>{String(playable.length).padStart(2,"0")} / 04 ITEMS</span></header><div className={`look-compose ${playable.length?"has-items":"is-empty"}`}><div className="look-silhouette" aria-hidden="true"><i/><i/><i/></div>{CATEGORIES.map(cat=>{const item=playable.find(product=>product.category===cat);return item?<figure key={item.id} className={`look-layer ${cat.toLowerCase()}`}><img src={item.image} alt={item.name}/><figcaption><small>{cat}</small><b>{item.name}</b></figcaption></figure>:<div key={cat} className={`look-slot ${cat.toLowerCase()}`}><span>+ {cat}</span></div>})}{!playable.length&&<div className="look-empty-copy"><span>NO ITEMS YET</span><p>{emptyCopy}</p></div>}</div><footer>CUTOUT LAYERS · LIVE OUTFIT PREVIEW</footer></section>
}
...
function Battle({player,onChoose,onBack}:{player:Product[];onChoose:(choice:"A"|"B")=>void;onBack:()=>void}){
 ...
 return <div className="play-page"><header className="screen-title"><span>⚔ STYLE BATTLE / ROUND 01</span><h1>YOUR LOOK<br/>VS. MUSE PICK!</h1><p>스탯을 비교하고 오늘의 승리 코디를 골라주세요.</p></header><div className="battle-guide"><img src="/ren-y2k-guide.png" alt="렌"/><p><b>REN&apos;S BATTLE CHECK</b><span>상품 태그를 분석해 두 LOOK의 무드와 공통 취향을 계산했어. 취향대로 선택해!</span></p></div><div className="battle-grid"><article><div className="battle-label"><span>PLAYER LOOK A</span><b>YOUR BUILD</b></div><LookBoard items={player} label="LOOK A / PLAYER 01"/><StyleBars variant="player" items={player}/><button onClick={()=>onChoose("A")}>LOOK A로 승부! →</button></article><div className="match-core"><span>MATCH POWER</span><strong>{matchScore}<small>%</small></strong><i>COMBO × {String(player.length).padStart(2,"0")}<br/>SHARED TAG × {String(shared).padStart(2,"0")}</i></div><article><div className="battle-label"><span>CHALLENGER B</span><b>MUSE PICK</b></div><LookBoard items={recommended} label="LOOK B / MUSE PICK"/><StyleBars variant="recommend" items={recommended}/><button onClick={()=>onChoose("B")}>LOOK B로 승부! →</button></article></div></div>
}
...
function Reveal({items,choice,onShop,onBack}:{items:Product[];choice:"A"|"B"|null;onShop:()=>void;onBack:()=>void}){
 ...
 return <div className="play-page"><header className="screen-title"><span>✦ REVEAL / LEVEL CLEAR!</span><h1>당신의 스타일은<br/>이거였어!</h1><p>선택한 조합에서 오늘의 취향 태그를 발견했어요.</p></header><div className="reveal-layout"><LookBoard items={items} label={`WINNING LOOK ${choice}`}/><aside><div className="level-clear">LEVEL CLEAR ★</div><span>FINAL MATCH SCORE</span><strong>{finalScore}<small>%</small></strong><h2>Y2K MINIMAL<br/>+ VINTAGE EDGE</h2><Tags items={tags}/><StyleBars variant={choice==="A"?"player":"recommend"} items={items}/><div className="reveal-products">{items.map(item=><div key={item.id}><span>{item.brand} · {item.category}</span><b>{item.name}</b><strong>{won(item.price)}</strong></div>)}</div><div className="reveal-total"><span>TOTAL PRICE</span><b>{won(total(items))}</b></div><div className="guide-tip reveal-tip"><img src="/miyu-y2k-guide.png" alt="미유"/><p><b>MIYU SAYS</b><span>이 조합, 완전 당신 취향이야! 저장하고 쇼핑으로 이어가볼까?</span></p></div><button onClick={onShop}>보상 LOOK 쇼핑하기 →</button></aside></div></div>
}
