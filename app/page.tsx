...
function Home({go}:{go:(s:Screen)=>void}){
 ...
 return <div className="page home-page">
  ...
  <section className="more-play"><SectionHead eyebrow="FUTURE PLAY MODES" title="MORE WAYS TO PLAY"/><div>{["BEAUTY PLAY","GUESS THE LOOK","THIS OR THAT"].map(x=><article key={x}><span>COMING SOON</span><b>{x}</b><p>새로운 선택 방식으로 확장됩니다.</p></article>)}</div><aside className="ren-host"><img src="/ren-mascot-3d-v6.png" alt="3D 플레이 호스트 렌"/><div><span>PLAY HOST · REN</span><b>NEXT MISSION<br/>IS COMING.</b><p>REN은 다음 PLAY의 호스트로 등장합니다.</p></div></aside></section>
