...
export default function Muse(){
 ...
 const startMission=(target:StylingFor)=>{setStylingFor(target);setSelectedIds([]);setSubmitted(false);go("build")};
 ...
 return <main><PlayHeader screen={screen} back={()=>exitPlay()}/>{screen==="home"&&<Home go={go} startMission={startMission}/>} {screen==="build"&&<Build stylingFor={stylingFor} setStylingFor={setStylingFor} selectedIds={selectedIds} setSelectedIds={setSelectedIds} submitted={submitted} setSubmitted={setSubmitted} go={go}/>} {screen==="battle"&&<Battle playerLook={playerLook} pick={battlePick} setPick={setBattlePick} go={go}/>} {screen==="shop"&&<Shop items={shopItems} returnToCommerce={exitPlay}/>}</main>
}

function Home({go,startMission}:{go:(s:Screen)=>void;startMission:(target:StylingFor)=>void}){
 return <div className="page home-page"><section className="home-hero"><div className="hero-copy"><span className="eyebrow">MUSE PLAY</span><h2>STYLE THE<br/>MOMENT.</h2><p>실제 판매 상품으로<br/>오늘의 LOOK을 만들어보세요.</p><button className="btn lime" onClick={()=>startMission("women")}>오늘의 미션 시작하기 →</button></div><div className="hero-board"><AutoLook items={byIds(["cardigan","top","denim","sneakers","bag"])} blind label="WHAT WOULD YOU WEAR?"/></div></section><section className="mission-feature"><div className="mascot mascot-3d"><img src="/miyu-mascot-3d-v4.png" alt="3D 미션 가이드 미유"/><span>MISSION GUIDE · MIYU</span></div><div><span className="eyebrow">MIYU&apos;S DAILY MISSION</span><h2>금요일 7PM,<br/>갑자기 잡힌 첫 소개팅</h2><blockquote>20만원 안으로 편안하지만 기억에 남는<br/>성수 데이트 LOOK.</blockquote><div className="chips"><span>29°C</span><span>TOP + BOTTOM + SHOES</span><span>🎴 NO BLACK</span></div><button className="btn purple" onClick={()=>startMission("women")}>미션 시작하기 →</button></div><aside><small>BUDGET</small><b>₩200,000</b><hr/><small>REQUIRED</small><p>TOP + BOTTOM + SHOES</p><hr/><small>TODAY&apos;S WILD CARD</small><p>🎴 NO BLACK<br/>검은색 아이템 없이 완성하세요.</p></aside></section><section className="ren-host"><img src="/ren-mascot-3d-final.png" alt="3D 미션 가이드 렌"/><div><span>NEXT MISSION · REN</span><b>주말 오후 2PM,<br/>한강 피크닉</b><p>렌과 함께 남성·유니섹스 아이템만 비교하며<br/>가볍고 편한 주말 LOOK을 만들어보세요.</p><button className="btn lime" onClick={()=>startMission("men")}>렌의 미션 시작하기 →</button></div><aside><small>STYLING FOR</small><strong>MEN</strong><i>남성 + 유니섹스</i><hr/><small>HIDDEN ITEMS</small><p>SKIRT · DRESS · HEELS</p></aside></section><section className="community-battle"><div><span>COMMUNITY BATTLE</span><h2>12,421명이 오늘의 LOOK을<br/>선택하고 있어요.</h2><button className="btn dark" onClick={()=>go("battle")}>투표하러 가기 →</button></div><section><AutoLook items={byIds(["cardigan","top","denim","sneakers"])} blind label="LOOK A"/><b>VS</b><AutoLook items={byIds(["shirt","cargo","canvas-shoes","red-scarf"])} blind label="LOOK B"/></section></section></div>
}
...
function Build({stylingFor,setStylingFor,selectedIds,setSelectedIds,submitted,setSubmitted,go}:{stylingFor:StylingFor;setStylingFor:(v:StylingFor)=>void;selectedIds:string[];setSelectedIds:(v:string[])=>void;submitted:boolean;setSubmitted:(v:boolean)=>void;go:(s:Screen)=>void}){
 ...
 const next=()=>{if(ready){setSubmitted(true)}else if(stage<2){setStage(stage+1);setShuffle(0)}};
 ...
 return <div className="page build-choice-page"><SectionHead eyebrow="DAILY MISSION" title={current.title} copy="MUSE가 고른 6개 중 하나를 선택하세요. TOP · BOTTOM · SHOES는 언제든 오가며 비교할 수 있어요."/><section className="styling-for" aria-label="코디 대상 선택"><div><span>STYLING FOR</span><b>{stylingFor==="women"?"WOMEN · MIYU":"MEN · REN"}</b></div><button className={stylingFor==="women"?"active":""} onClick={()=>changeTarget("women")}><b>여성 코디</b><small>여성 + 유니섹스</small></button><button className={stylingFor==="men"?"active":""} onClick={()=>changeTarget("men")}><b>남성 코디</b><small>남성 + 유니섹스</small></button><p>{stylingFor==="men"?"스커트·원피스·하이힐은 표시하지 않아요.":"여성 무드와 유니섹스 상품만 보여드려요."}</p></section><section className="mission-bar"><div><small>FRIDAY · 7PM</small><b>갑자기 잡힌 첫 소개팅</b><span>SEOUL · SEONGSU · 29°C</span></div><div><small>BUDGET</small><b>₩200,000</b></div><div><small>TODAY&apos;S WILD CARD</small><b>🎴 NO BLACK</b><span>검은색 아이템 없이 완성하세요.</span></div></section><div className="choice-progress">{steps.map((item,index)=><button key={item.category} className={index===stage?"active":cats.has(item.category)?"done":""} onClick={()=>{setStage(index);setShuffle(0)}} aria-pressed={index===stage}><span>0{index+1}</span><b>{item.category}</b><i>{cats.has(item.category)?"✓":""}</i></button>)}</div><div className="choice-builder"><section className="choice-products"><header><span>{current.title}</span><b>6 ITEMS</b></header><div>{choices.map(item=><button key={item.id} className={picked?.id===item.id?"selected":""} onClick={()=>choose(item)}><img src={item.image} alt={item.name}/><small>{item.brand}</small><b>{item.name}</b><span>{won(item.sale??item.price)}</span><i>{picked?.id===item.id?"SELECTED":"PICK +"}</i></button>)}</div><footer><button className="shuffle-btn" onClick={()=>setShuffle(shuffle+2)}>마음에 드는 게 없나요?　↻ SHUFFLE</button><button className="btn purple" disabled={!ready&&!picked} onClick={next}>{ready?"LOOK 완성하기 →":stage<2?`다음: ${steps[stage+1].category} 고르기 →`:!noBlack?"BLACK 아이템을 바꿔주세요":"미션 조건을 확인해주세요"}</button></footer></section><aside className="choice-summary"><AutoLook items={selected}/><div className="choice-budget"><span>BUDGET</span><b>{won(amount)} <small>/ {won(budget)}</small></b><i style={{width:`${Math.min(100,amount/budget*100)}%`}}/><strong className={left<0?"over":""}>{left>=0?`${won(left)} LEFT`:`${won(Math.abs(left))} OVER`}</strong></div><div className={noBlack?"wild-card clear":"wild-card fail"}><span>TODAY&apos;S WILD CARD</span><b>🎴 NO BLACK {noBlack?"✓":""}</b></div></aside></div></div>
}
