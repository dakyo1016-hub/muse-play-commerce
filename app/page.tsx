...
function Home({go}:{go:(s:Screen)=>void}){
 ...
 return <div className="page home-page">
  ...
  <section className="mission-feature"><div className="mascot mascot-3d"><img src="/miyu-mascot-3d-v2.png" alt="3D 미션 가이드 미유"/><span>MISSION GUIDE · MIYU</span></div><div><span className="eyebrow">MIYU&apos;S DAILY MISSION</span><h2>금요일 7PM,<br/>갑자기 잡힌 첫 소개팅</h2><blockquote>“20만원 안으로 편안하지만 기억에 남는<br/>성수 데이트 룩을 만들어줄래?”</blockquote><div className="chips"><span>29°C</span><span>TOP + BOTTOM + SHOES</span><span>🎴 NO BLACK</span></div><button className="btn purple" onClick={()=>go("build")}>미션 수락하기 →</button><small className="helper-copy">HELP MIYU · 오늘의 LOOK을 완성해주세요.</small></div><aside><small>BUDGET</small><b>₩200,000</b><hr/><small>SPECIAL RULE</small><p>NO BLACK<br/>검은색 아이템 없이 완성하세요.</p></aside></section>
  ...
  <section className="more-play"><SectionHead eyebrow="FUTURE PLAY MODES" title="MORE WAYS TO PLAY"/><div>{["BEAUTY PLAY","GUESS THE LOOK","THIS OR THAT"].map(x=><article key={x}><span>COMING SOON</span><b>{x}</b><p>새로운 선택 방식으로 확장됩니다.</p></article>)}</div><aside className="ren-host"><img src="/ren-mascot-3d-v3.png" alt="3D 플레이 호스트 렌"/><div><span>PLAY HOST · REN</span><b>NEXT MISSION<br/>IS COMING.</b><p>REN은 다음 PLAY의 호스트로 등장합니다.</p></div></aside></section>
...
function Build({category,setCategory,selectedIds,setSelectedIds,submitted,setSubmitted,go}:{category:Category;setCategory:(c:Category)=>void;selectedIds:string[];setSelectedIds:(v:string[])=>void;submitted:boolean;setSubmitted:(v:boolean)=>void;go:(s:Screen)=>void}){
 ...
 const baseVisible=category==="ALL"?PRODUCTS:PRODUCTS.filter(x=>x.category===category),visible=priceCap===null?baseVisible:baseVisible.filter(x=>(x.sale??x.price)<=priceCap);
 ...
 return <div className="page"><SectionHead eyebrow="CORE GAME 01 · BUILD A LOOK" title="TODAY'S MISSION" copy="실제 판매 상품을 골라 미션에 맞는 LOOK을 완성하세요. Flat Lay는 자동으로 생성됩니다."/>
  ...
  <div className="category-tabs">{CATEGORIES.map(c=><button className={category===c?"active":""} onClick={()=>{setCategory(c);setPriceCap(null)}} key={c}>{c}<span>{c==="ALL"?PRODUCTS.length:PRODUCTS.filter(x=>x.category===c).length}</span></button>)}</div>
...
function Shop({alternative,setAlternative,bagIds,setBagIds,notify}:{alternative:boolean;setAlternative:(v:boolean)=>void;bagIds:string[];setBagIds:(v:string[])=>void;notify:(s:string)=>void}){
 ...
 return <div className="page shop-page"><SectionHead eyebrow="SHOP" title="SHOP THE LOOK" copy="게임은 끝났습니다. 선택한 상품을 확인하고 익숙한 방식으로 구매하세요."/><div className="shop-layout shop-commerce"><section><AutoLook items={items} label={alternative?"SIMILAR LOOK":"SELECTED LOOK"}/></section><aside><span className="eyebrow">{alternative?"LOWER PRICE ALTERNATIVES":"SELECT ITEMS"}</span><h2>{alternative?"비슷한 무드의 대체 상품":"선택한 LOOK의 상품"}</h2><div className="shop-products">{items.map(item=><label key={item.id}><input type="checkbox" checked={bagIds.includes(item.id)} onChange={()=>toggle(item.id)}/><img src={item.image} alt=""/><span><small>{item.brand} · {item.category}</small><b>{item.name}</b></span><strong>{won(item.sale??item.price)}</strong></label>)}</div><div className="shop-total"><span>TOTAL · {active.length} ITEMS</span><b>{won(total(active))}</b></div><button className="commerce-cta" disabled={!active.length} onClick={()=>notify(`${active.length}개 상품을 장바구니에 담았습니다.`)}>선택한 상품 장바구니 담기</button><button className="alternative-link" onClick={()=>{setAlternative(!alternative);setBagIds((alternative?original:alt).map(x=>x.id))}}>{alternative?"원래 LOOK 보기":"LOVE THE LOOK, NOT THE PRICE? · 저렴한 대체 상품 보기 →"}</button></aside></div></div>
}
