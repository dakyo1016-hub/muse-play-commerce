...
function LookBoard({items,label="SELECTED LOOK",emptyCopy="카테고리별 상품을 선택하면 이곳에 LOOK이 쌓입니다.",audience}:{items:Product[];label?:string;emptyCopy?:string;audience?:Audience}){
 ...
 return <section className="look-board"><header><b>{label}</b><span>{String(playable.length).padStart(2,"0")} / 04 ITEMS</span></header><div className={`look-compose ${playable.length?"has-items":"is-empty"}`}><img className="look-model" src={player==="men"?"/ren-fullbody-v5.png":"/miyu-fullbody-v5.png"} alt={player==="men"?"렌 전신 캐릭터":"미유 전신 캐릭터"}/>{CATEGORIES.map(cat=>{const item=playable.find(product=>product.category===cat);return item?<figure key={item.id} className={`look-layer ${cat.toLowerCase()}`}><img src={item.image} alt={item.name}/><figcaption><small>{cat}</small><b>{item.name}</b></figcaption></figure>:<div key={cat} className={`look-slot ${cat.toLowerCase()}`}><span>+ {cat}</span></div>})}{!playable.length&&<div className="look-empty-copy"><span>START YOUR LOOK</span><p>{emptyCopy}</p></div>}</div><footer>FULL BODY · CUTOUT LAYER PREVIEW</footer></section>
}
...
function Shop({initialItems,audience,onReturn,onBack}:{initialItems:Product[];audience:Audience;onReturn:(ids:string[])=>void;onBack:()=>void}){
 ...
 return <div className="play-page"><header className="screen-title shop-title"><span>FINAL STAGE / SHOP</span><h1>GET THE<br/>WINNING LOOK</h1><p>코디 세트를 한 번에 담거나 원하는 아이템만 추가하세요.</p></header><div className="shop-layout"><LookBoard items={items} label="FINAL LOOK / INVENTORY" audience={audience}/><aside><header><span>CHOOSE YOUR LOOK ITEMS</span><b>{active.length} / {items.length}</b></header><div className="shop-list">{items.map(item=><article key={item.id} className={!item.stock?"unavailable":""}><input type="checkbox" aria-label={`${item.name} 선택`} checked={selected.includes(item.id)} disabled={!item.stock} onChange={()=>setSelected(selected.includes(item.id)?selected.filter(id=>id!==item.id):[...selected,item.id])}/><img src={item.image} alt=""/><div><small>{item.brand} · {item.category}</small><b>{item.name}</b><Tags items={item.tags.slice(0,2)}/></div><strong>{won(item.price)}</strong>{item.stock?<button onClick={()=>onReturn([item.id])}>+ 개별 추가</button>:<button className="swap" onClick={()=>swap(item)}>비슷한 스타일 추천</button>}{!item.stock&&<p>품절 아이템이에요! 같은 태그의 대체 아이템을 불러왔어요.</p>}</article>)}</div><div className="shop-summary"><span>SET TOTAL · {active.length} ITEMS</span><b>{won(total(active))}</b></div><button className="tryon-cta" disabled={active.length!==4} onClick={()=>window.location.assign(`/try-on?items=${active.map(item=>item.id).join(",")}`)}>내 사진에 이 LOOK 입혀보기 →</button><button className="bulk-cta" disabled={!active.length} onClick={()=>onReturn(active.map(item=>item.id))}>코디 세트 한 번에 담기 →</button><p className="handoff-note">CHECKOUT은 기존 커머스에서 안전하게 이어집니다.</p></aside></div></div>
}

export function LegacyMuse({initialScreen="commerce"}:{initialScreen?:Screen}){
 ...
 const exit=(ids:string[]=[])=>{if(initialScreen!=="commerce"){window.location.assign(ids.length?`/bag?cart=${ids.join(",")}`:"/");return}setCartIds(ids);setScreen("commerce");window.scrollTo({top:0,behavior:"smooth"})};
 if(screen==="commerce")return <><header className="commerce-nav"><button className="logo" onClick={()=>{setCartIds([]);window.location.assign("/")}}>MUSE <span>— SHOPPING AS A GAME</span></button><nav><button onClick={()=>window.location.assign("/items")}>ITEMS</button><button onClick={()=>window.location.assign("/ranking")}>RANKING</button><button className="play-badge" onClick={enter}>★ PLAY <span>HOT</span></button><button onClick={()=>window.location.assign("/try-on")}>AI TRY-ON</button><button onClick={()=>window.location.assign("/quest")}>QUEST</button><button onClick={()=>window.location.assign("/my")}>MY</button></nav><button className="bag" onClick={()=>window.location.assign(cartIds.length?`/bag?cart=${cartIds.join(",")}`:"/bag")}>BAG {cartIds.length}</button></header><CommerceHome enter={enter} cartItems={byIds(cartIds)} clearCart={()=>setCartIds([])}/></>;
...
export default function GodotSite(){
 return <main className="godot-site-host"><iframe src="/game/muse-v50.html" title="MUSE — Shopping as a Game" allow="autoplay; fullscreen; gamepad" loading="eager"/><a className="godot-tryon-entry" href="/try-on" target="_top"><span>NEW</span><b>AI 피팅</b><i>→</i></a></main>;
}
