...
function Home({go}:{go:(s:Screen)=>void}){
 ...
 return <div className="page home-page">
  ...
  <section className="mission-feature"><div className="mascot"><img src="/characters/miyu-starter/front.png" alt="미션 가이드 미유"/><span>MISSION GUIDE · MIYU</span></div><div><span className="eyebrow">MIYU&apos;S DAILY MISSION</span><h2>금요일 7PM,<br/>갑자기 잡힌 첫 소개팅</h2><blockquote>“20만원 안으로 편안하지만 기억에 남는<br/>성수 데이트 룩을 만들어줄래?”</blockquote><div className="chips"><span>29°C</span><span>TOP + BOTTOM + SHOES</span><span>🎴 NO BLACK</span></div><button className="btn purple" onClick={()=>go("build")}>미션 수락하기 →</button><small className="helper-copy">HELP MIYU · 오늘의 LOOK을 완성해주세요.</small></div><aside><small>BUDGET</small><b>₩200,000</b><hr/><small>SPECIAL RULE</small><p>NO BLACK<br/>검은색 아이템 없이 완성하세요.</p></aside></section>
  ...
  <section className="more-play"><SectionHead eyebrow="FUTURE PLAY MODES" title="MORE WAYS TO PLAY"/><div>{["BEAUTY PLAY","GUESS THE LOOK","THIS OR THAT"].map(x=><article key={x}><span>COMING SOON</span><b>{x}</b><p>새로운 선택 방식으로 확장됩니다.</p></article>)}</div></section>
