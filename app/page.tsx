...
function Battle({player,onChoose,onBack}:{player:Product[];onChoose:(choice:"A"|"B")=>void;onBack:()=>void}){
 ...
 return <div className="play-page"><header className="screen-title"><span>⚔ STYLE BATTLE / ROUND 01</span><h1>YOUR LOOK<br/>VS. MUSE PICK!</h1><p>스탯을 비교하고 오늘의 승리 코디를 골라주세요.</p></header><div className="battle-guide"><img src="/ren-y2k-guide.png" alt="렌"/><p><b>REN&apos;S BATTLE CHECK</b><span>LOOK A는 스트릿 스탯이 강하고, LOOK B는 미니멀 밸런스가 높아. 취향대로 선택해!</span></p></div><div className="battle-grid"><article><div className="battle-label"><span>PLAYER LOOK A</span><b>YOUR BUILD</b></div><LookBoard items={player} label="LOOK A / PLAYER 01"/><StyleBars variant="player"/><button onClick={()=>onChoose("A")}>LOOK A로 승부! →</button></article><div className="match-core"><span>MATCH POWER</span><strong>84<small>%</small></strong><i>COMBO × 04<br/>SHARED TAG × 03</i></div><article><div className="battle-label"><span>CHALLENGER B</span><b>MUSE PICK</b></div><LookBoard items={recommended} label="LOOK B / MUSE PICK"/><StyleBars variant="recommend"/><button onClick={()=>onChoose("B")}>LOOK B로 승부! →</button></article></div></div>
}
...
function Reveal({items,choice,onShop,onBack}:{items:Product[];choice:"A"|"B"|null;onShop:()=>void;onBack:()=>void}){
 ...
 return <div className="play-page"><header className="screen-title"><span>✦ REVEAL / LEVEL CLEAR!</span><h1>당신의 스타일은<br/>이거였어!</h1><p>선택한 조합에서 오늘의 취향 태그를 발견했어요.</p></header><div className="reveal-layout"><LookBoard items={items} label={`WINNING LOOK ${choice}`}/><aside><div className="level-clear">LEVEL CLEAR ★</div><span>FINAL MATCH SCORE</span><strong>88<small>%</small></strong><h2>Y2K MINIMAL<br/>+ VINTAGE EDGE</h2><Tags items={tags}/><StyleBars variant={choice==="A"?"player":"recommend"}/><div className="reveal-products">{items.map(item=><div key={item.id}><span>{item.brand} · {item.category}</span><b>{item.name}</b><strong>{won(item.price)}</strong></div>)}</div><div className="guide-tip reveal-tip"><img src="/miyu-y2k-guide.png" alt="미유"/><p><b>MIYU SAYS</b><span>이 조합, 완전 당신 취향이야! 저장하고 쇼핑으로 이어가볼까?</span></p></div><button onClick={onShop}>보상 LOOK 쇼핑하기 →</button></aside></div></div>
}
